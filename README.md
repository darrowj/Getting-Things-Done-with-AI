...
![Daily brief architecture: Google Calendar, Gmail and three Notion databases feed one Python job on a weekday cron. A narrow regex plus one batched local-model call filters the inbox. A fixed priority ladder in Python then decides which single item matters most, with no model involved, before the model is asked to phrase it. The result is one JSON file the Next.js planner renders.](docs/architecture.png)

Green decides, red only writes.

The model never chooses the priority. `deterministic_most_important()` is a fixed
ladder in plain Python: a HIGH-priority task outranks a live job stage, which
outranks an interview on the calendar, down through overdue tasks and urgent mail
to the first event of the day. Only once Python has picked the winner is the model
handed that one item and asked to phrase it.

There are four model calls in total (inbox triage, the sentence, prep notes, the
narrative paragraph). Every one has a deterministic fallback, so a model that is
slow or down degrades the brief rather than breaking it. The inbox filter is
...
- **Daily Brief** — read-only morning brief from Google Calendar, Gmail, and three Notion databases. Python picks the single most important item from a fixed priority ladder; the local Ollama model phrases it and writes the prep notes and narrative. Written to `data/daily-brief.json` each weekday; no DB table or history.
...
docs/
├── make_architecture.py     # Generates the diagram — edit this, not the SVG
├── architecture.svg
└── architecture.png
```
...
rsync -av \
  ...
  ./ darrowj@homelab:~/development/gtd-app/
...
Access from any device on the network: `http://homelab:3000`
...
The planner and the brief generator are both worth forking.  `data/gtd.db`,
`data/daily-brief.json` and `.env` are all gitignored, so you get the code and
none of my calendar.
